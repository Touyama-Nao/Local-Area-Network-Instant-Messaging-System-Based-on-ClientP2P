...
export default {
  ...
  mounted() {
    ...
    function createCircle() {
      for (var i = 0; i < that.dots.n; i++) {
        that.dots.array.push(new Dot());
      }
    }
    //绘制出圆形粒子
    function drawDots() {
      for (var i = 0; i < that.dots.n; i++) {
        var dot = that.dots.array[i];
        dot.draw();
      }
    }
    ...
    function connectDots() {
      for (var i = 0; i < that.dots.n; i++) {
        for (var j = 0; j < that.dots.n; j++) {
          var iDot = that.dots.array[i];
          var jDot = that.dots.array[j];
          if (
            iDot.x - jDot.x < that.dots.distance &&
            iDot.y - jDot.y < that.dots.distance &&
            iDot.x - jDot.x > -that.dots.distance &&
            iDot.y - jDot.y > -that.dots.distance
          ) {
            if (
              iDot.x - mousePosition.x < that.dots.d_radius &&
              iDot.y - mousePosition.y < that.dots.d_radius &&
              iDot.x - mousePosition.x > -that.dots.d_radius &&
              iDot.y - mousePosition.y > -that.dots.d_radius
            ) {
              cxt.beginPath();
